...
import { ArrowUpRight, Search, PhoneCall, Mail, MessageSquare, CheckCircle2, ChevronDown, MapPin } from 'lucide-react';
...
export function HelpCenterView({ onOpenBooking }: HelpCenterViewProps) {
    ...
    <div className="pt-24 pb-20 px-6 md:px-12 max-w-7xl mx-auto animate-fade-in text-slate-900">
      ...
      <div className="text-center max-w-3xl mx-auto mb-12">
        ...
        <div className="relative max-w-xl mx-auto mb-8">
          ...
          <input
            type="text"
            placeholder="Search departures, packing lists, group discounts, trek routes..."
            value={searchQuery}
                  ...
                  return (
                    <div key={qIdx} className="border border-slate-100 rounded-2xl overflow-hidden bg-slate-50/50">
                      <button
                        ...
                        className="w-full px-5 py-3.5 flex items-center justify-between text-left font-bold text-xs sm:text-sm text-slate-800 hover:text-slate-950"
                      >
                        <span>{item.q}</span>
          ...
          {contactSubmitted ? (
            ...
            <form onSubmit={handleSendMessage} className="space-y-3">
              <textarea
                required
                rows={3}
                placeholder="Ask about gear specifications, private group charters, customized itineraries..."
                value={message}
              ...
              <button
                type="submit"
                className="bg-white text-slate-950 hover:bg-slate-100 px-7 py-3 rounded-full text-xs font-bold transition-all shadow-md active:scale-95"
              >
